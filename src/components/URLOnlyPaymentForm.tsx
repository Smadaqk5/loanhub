...
const urlPaymentSchema = z.object({
...
type URLPaymentFormData = z.infer<typeof urlPaymentSchema>

interface URLOnlyPaymentFormProps {
  loanId: string
  userId: string
...
export function URLOnlyPaymentForm({
  loanId,
  userId,
  processingFeeAmount,
  onPaymentSuccess,
  ...
  const {
    ...
    watch,
    formState: { errors }
  } = useForm<URLPaymentFormData>({
    resolver: zodResolver(urlPaymentSchema),
    defaultValues: {
  ...
  const onSubmit = async (data: URLPaymentFormData) => {
    setIsLoading(true)
